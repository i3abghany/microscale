...
# This file is taken from the MLPerf Tiny repository. All rights reserved to the
# original authors.  The original file can be found at
# https://github.com/mlcommons/tiny -- The license of the original code is
# included under the LICENSES directory in this repository.
...
def get_training_data(config, get_waves=False, val_cal_subset=False):
  
  label_count=12
  background_frequency = config['background_frequency']
  background_volume_range_= config['background_volume']
  model_settings = models.prepare_model_settings(label_count, config)

  bg_path = os.path.join(os.getenv('HOME'), config['bg_path'])
  BACKGROUND_NOISE_DIR_NAME='_background_noise_' 
  background_data = prepare_background_data(bg_path,BACKGROUND_NOISE_DIR_NAME)

  splits = ['train', 'test', 'validation']
  dir_ = os.path.join(os.getenv('HOME'), config['download_dir'])
  (ds_train, ds_test, ds_val), ds_info = tfds.load('speech_commands', split=splits, 
                                                data_dir=dir_, with_info=True)
  ...
  if config['num_train_samples'] != -1:
    ds_train = ds_train.take(config['num_train_samples'])
  if config['num_val_samples'] != -1:
    ds_val = ds_val.take(config['num_val_samples'])
  if config['num_test_samples'] != -1:
    ds_test = ds_test.take(config['num_test_samples'])
